...
//  hints.ai_family = AF_INET6;
  ...
  if (ret != 0) {
    OATPP_LOGE("[oatpp::network::server::SimpleTCPConnectionProvider::instantiateServer()]",
               "Error. Call to getaddrinfo() failed with result=%d: (errno:%d, %s)", ret, errno, strerror(errno));
    throw std::runtime_error("[oatpp::network::server::SimpleTCPConnectionProvider::instantiateServer()]: Error. Call to getaddrinfo() failed.");
  }
...
//  OATPP_LOGD("[SimpleTCPConnectionProvider::instantiateServer()]", "(result->ai_family=%d, result->ai_socktype=%d, result->ai_protocol=%d)", result->ai_family, result->ai_socktype, result->ai_protocol);
  ...
  if (serverHandle < 0) {
    OATPP_LOGE("[oatpp::network::server::SimpleTCPConnectionProvider::instantiateServer()]",
               "Error. Couldn't open a socket: socket(%d, %d, %d): (errno: %d, %s)",
               result->ai_family, result->ai_socktype, result->ai_protocol, errno, strerror(errno));
  ...
  if(ret < 0) {
    OATPP_LOGE("[oatpp::network::server::SimpleTCPConnectionProvider::instantiateServer()]",
               "Warning. Failed to set %s for accepting socket: (errno: %d, %s)", "SO_REUSEADDR", errno, strerror(errno));
  }
  ...
  if(ret != 0) {
    ::close(serverHandle);
    OATPP_LOGE("[oatpp::network::server::SimpleTCPConnectionProvider::instantiateServer()]",
               "Error. Failed to bind port %d: (errno: %d, %s)", m_port, errno, strerror(errno));
